...
final class HeadersDownloader
{
    ...
    {
        return \vsprintf('https://github.com/glfw/glfw/archive/refs/tags/%s.zip', [
            \str_ends_with($version->toString(), '.0') ? \substr($version->toString(), 0, -2) : $version->toString(),
        ]);
    }
    ...
    {
        return \sys_get_temp_dir() . '/glfw3-headers-' . $version->toString() . '.zip';
    }
    ...
    public static function download(VersionInterface $version, string $directory): void
    {
        ...
        foreach (self::readArchive($version) as $file) {
            if ($file->isFile() && \str_contains($file->getPathname(), 'include/GLFW')) {
